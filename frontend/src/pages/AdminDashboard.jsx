...
const AdminDashboard = () => {
    ...
    const fetchData = async () => {
        try {
            const [eventsRes, bookingsRes] = await Promise.all([
                api.get('/events'),
                api.get('/bookings/my-booking') // Admin gets all bookings
            ]);
    ...
    const handleCreateEvent = async (e) => {
        ...
        try {
            ...
            const errorMsg = error.response?.data?.message || error.response?.data?.error || 'Error creating event';
    ...
    const handleUpdatePaymentStatus = async (id, newStatus) => {
        try {
            await api.put(`/bookings/${id}/payment-status`, { paymentStatus: newStatus });
            fetchData();
        } catch (error) {
                            ...
                            {bookings.length === 0 ? <li className="p-6 text-gray-500 text-center">No bookings yet.</li> :
                                bookings.map(booking => (
                                    <li key={booking._id} className={`p-6 hover:bg-gray-50 transition border-l-4 ${booking.status === 'pending' ? 'border-l-yellow-400' : booking.status === 'confirmed' ? 'border-l-green-400' : 'border-l-red-400'}`}>
                                        ...
                                        {booking.status === 'confirmed' && booking.paymentStatus === 'not_paid' && (
                                            <div className="flex flex-wrap gap-2 mt-2">
                                                <button onClick={() => handleUpdatePaymentStatus(booking._id, 'paid')} className="w-full bg-green-50 text-green-700 hover:bg-green-600 hover:text-white border border-green-200 text-xs font-bold py-2.5 px-3 rounded-lg shadow-sm transition">
                                                    💳 Mark as Paid
                                                </button>
                                            </div>
                                        )}
